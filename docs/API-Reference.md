...
Dimina 的 render runtime 使用 Vue，但对外目标仍是小程序语义。以下是当前尚未完全抹平、迁移时需要特别检查的差异：

- **具名插槽的显示逻辑**

    微信小程序允许多个同名具名插槽内容同时渲染，而 Vue 默认会覆盖同名插槽。当前使用 `<text slot="info">成功</text><text slot="info">失败</text>` 时，后一个节点可能不会显示；迁移前需要验证此类组件。

- **组件属性的默认值**

    小程序与 Vue 对布尔属性空字符串的解析不同。字符串类型属性未赋值时，微信小程序会回退为空字符串并提示类型不兼容；Dimina 当前可能保留传入值。组件不要同时依赖隐式布尔转换和空字符串回退。

- **循环内的访问限制**

    微信小程序允许同一节点上的 `wx:if` 访问 `wx:for` 当前项，而 Vue 转换后的作用域规则不同。遇到此组合时，应把条件判断移动到循环节点的子节点，并通过真机或 Web 示例确认渲染结果。
