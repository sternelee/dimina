 ...
 */
interface MiniAppStub {
	apiRegistry: Record<string, (this: MiniAppStub, params?: unknown) => void>
	registerApi: (name: string, handler: (this: MiniAppStub, params?: unknown) => void) => void
	// 模拟第三方扩展路由
	_handleExtCall: (name: string, params?: unknown) => void
	invokeApi: (name: string, params?: unknown) => void
	// 允许桩造/断言任意内置方法名（如 request），复刻 MiniApp.invokeApi 的动态派发
	[key: string]: unknown
}

function createMiniAppStub(): MiniAppStub {
	const app: MiniAppStub = {
		apiRegistry: {},
		...
		invokeApi(name, params) {
			...
			if (handler) {
				...
				(this[name] as (params?: unknown) => void)(params)
			}
			else {
...
describe('MiniApp registerApi', () => {
	...
	it('should call handler with app as this context', () => {
		const app = createMiniAppStub()
		let thisRef: MiniAppStub | null = null
