...
function mockProcessWxsRequires(wxsContent, wxsFilePath, filePath, workPath) {
	...
	const transformWxsCode = (code) => {
		const ast = parseSync('mock.wxs', code, {
			sourceType: 'script',
			lang: 'js',
		}).program
		...
		walk(ast, {
			...
			// getRegExp -> new RegExp, getDate -> new Date
				if (
					...
					&& (node.callee?.name === 'getRegExp' || node.callee?.name === 'getDate')
				) {
					const calleeName = node.callee.name.substring(3)
					const args = node.arguments.map(arg => code.slice(arg.start, arg.end)).join(', ')
					replacements.push({
						start: node.start,
						end: node.end,
						value: `new ${calleeName}(${args})`,
					})
				}
			}
		})
	...
	const wxsAst = parseSync('mock.wxs', wxsContent, {
		sourceType: 'script',
		lang: 'js',
	}).program
					...
					if (filePath.includes('/miniprogram_npm/')) {
						...
						if (fs.existsSync(resolvedWxsPath)) {
							...
							if (depWxsContent && !scriptModule.find(sm => sm.path === moduleName)) {
								scriptModule.push({
									path: moduleName,
									code: transformWxsCode(depWxsContent),
								})
							}
						}
