...
class DMPResourceManager {
    ...
    public static func prepareApp(appId: String) {
        ...
        if FileManager.default.fileExists(atPath: bundlePath) {
            ...
            let configBundle = DMPFileUtil.loadJSONFromFile(filePath: resourcePath + "/\(appId)/config.json")
        ...
        if DMPFileUtil.createDirectory(at: bundlePath) {
            if let resourcePath = bundle?.resourcePath {
                ...
                let appZipPath = (appResourcePath as NSString).appendingPathComponent("\(appId).zip")
    ...
    public static func prepareSdk() {
        ...
        if FileManager.default.fileExists(atPath: sdkBundlePath) {
            let config = DMPFileUtil.loadJSONFromFile(filePath: DMPSandboxManager.sdkConfigPath())
            let versionCodeOld = config?["versionCode"] as? Int ?? 0

            // 加载 bundle 下的 config.json
            let resourcePath = (bundle?.resourcePath)!
            let configBundle = DMPFileUtil.loadJSONFromFile(filePath: resourcePath + "/config.json")
            let versionCodeNew = configBundle?["versionCode"] as? Int ?? 0
        ...
        if DMPFileUtil.createDirectory(at: sdkBundlePath) {
            ...
            if let resourcePath = bundle?.resourcePath {
                ...
                if !DMPFileUtil.fileExists(at: sandboxPath + "/assets") || !DMPFileUtil.fileExists(at: sandboxPath + "/pageFrame.html") {
    ...
    static var jsappBundle: Bundle? = {
        guard let bundleURL = Bundle.main.url(forResource: "JSAppBundle", withExtension: "bundle") else {
            return nil
        }
    ...
    static var jssdkBundle: Bundle? = {
        guard let bundleURL = Bundle.main.url(forResource: "JSSDKBundle", withExtension: "bundle") else {
            return nil
        }
        return Bundle(url: bundleURL)
    }()
    ...
    static func getDMPAppConfigs() -> [DMPAppConfig] {
        ...
        guard let jsappBundle = jsappBundle,
              let jsappPath = jsappBundle.resourcePath else {
            return appItems
        }
